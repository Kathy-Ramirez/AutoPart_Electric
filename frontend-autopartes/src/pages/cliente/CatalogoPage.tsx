    ...
    <div className="p-4 md:p-8 text-white">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        ...
        <select 
          ...
          className="p-3 rounded-xl bg-slate-900 w-full sm:w-auto"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {productosFiltrados.map((producto: any) => (
          ...
          >
            ...
            <div className="p-5">
              <h2 className="font-bold text-xl">{producto.nombre_producto}</h2>
              <p className="text-blue-200">{producto.marca} </p>
              <p className="mt-2"> Bs. {producto.precio} </p>
              <p> Stock: {producto.stock} </p>

              <button 
                className="mt-4 w-full bg-green-600 hover:bg-green-700 py-2 rounded-xl transition-all" 
                onClick={() => {
                  const mensaje = `Hola, deseo consultar disponibilidad:\n\nProducto: ${producto.nombre_producto}\nMarca: ${producto.marca}\nPrecio: Bs. ${producto.precio}\nCategoría: ${producto.categoria?.nombre_categoria || 'Eléctrico'}\n¿Está disponible?`;
